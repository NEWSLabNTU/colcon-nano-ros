...
class NrosTestTask(TaskExtensionPoint):
    ...
    async def test(self, *, additional_hooks=None):
        ...
        if platform == 'native':
            ...
            logger.warning(
                f"No test runner for platform '{platform}' — skipping")
            self._write_junit(test_results_path, pkg.name, 'skipped',
                              0, '', '')
            return 0

        passed = (rc == 0)
        self._write_junit(
            test_results_path, pkg.name,
    ...
    def _find_binary(self, lib_dir):
        ...
        for f in sorted(lib_dir.iterdir()):
            if f.is_file() and (os.access(str(f), os.X_OK) or
                                f.suffix in ('.elf', '.exe')):
                return f
        return None
